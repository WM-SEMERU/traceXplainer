...
@app.callback(
    ...
    Input('graphlink-type-dropdown', 'value'),
    State('local', 'data'))
...
@app.callback(
    ...
    Input("box-metric-dropdown", "value"),
...
def update_box_graph(metric, set_type, link_type, group, store_data):
    ...
    if box:
        box = "outliers"
    if "Linked?" in group:
        figure = px.box(df, y=metric, color_discrete_sequence=["red", "blue"], color="Linked?", points=box)
    else:
        figure = px.box(df, y=metric, color_discrete_sequence=["blue", "red"], points=box)
    figure.update_layout(
        title="Metric Boxplot"
    )
    return figure
...
@app.callback(
    Output("box-metric-dropdown", "options"),
    Output("box-metric-dropdown", "value"),
    Input('box-dropdown', "value"),
    Input("graphlink-type-dropdown", "value"),
    State('local', 'data'))
def update_box_metric_dropdown(set_type, link, data):
    params = {"system": data["params"]["system"],
              "experiment_path_w2v": data["vectors"]["word2vec" + "-" + link]["path"],
              "experiment_path_d2v": data["vectors"]["doc2vec" + "-" + link]["path"],
              "corpus": data["params"]["corpus"]
...
@app.callback(
    Output("lag-metric-dropdown", "options"),
    Output("lag-metric-dropdown", "value"),
    Input('lag-dropdown', "value"),
    Input("graphlink-type-dropdown", "value"),
    State('local', 'data'))
def update_lag_metric_dropdown(set_type, link, data):
    params = {"system": data["params"]["system"],
              "experiment_path_w2v": data["vectors"]["word2vec" + "-" + link]["path"],
              "experiment_path_d2v": data["vectors"]["doc2vec" + "-" + link]["path"],
              "corpus": data["params"]["corpus"]
...
@app.callback(
    Output("auto-metric-dropdown", "options"),
    Output("auto-metric-dropdown", "value"),
    Input('auto-dropdown', "value"),
    Input("graphlink-type-dropdown", "value"),
    State('local', 'data'))
def update_auto_metric_dropdown(set_type, link, data):
    params = {"system": data["params"]["system"],
              "experiment_path_w2v": data["vectors"]["word2vec" + "-" + link]["path"],
              "experiment_path_d2v": data["vectors"]["doc2vec" + "-" + link]["path"],
              "corpus": data["params"]["corpus"]
...
@app.callback(
    ...
    Input("auto-metric-dropdown", "value"),
    Input('auto-dropdown', "value"),
    Input('graphlink-type-dropdown', 'value'),
    State('local', 'data'))
...
@app.callback(
    ...
    State('local', 'data'))
def update_sim_graph(vec, link, store_data):
