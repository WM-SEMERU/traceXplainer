...
def dronology_params():
    ...
    return {
        ...
        'linkType': 'req2src',
        'path_to_trained_model': '../models/code_word2vec.model',
        'source_type': 'req',
        'target_type': 'code',
        'system_path_config': {
            'system_path': os.path.join(base, 'dronology_conv.csv'),
            'sep': '~',
            'prep': Preprocessing.conv,
            'names': ['ids', 'conv']
        },
        'saving_path': os.path.join(base, 'results') + os.sep,
...
class BasicSequenceVectorization:
    def __init__(self, params):
        ...
        tag = names[1]
        docs = [d.split() for d in self.df_all_system[self.df_all_system[tag] != ''][tag].values]
