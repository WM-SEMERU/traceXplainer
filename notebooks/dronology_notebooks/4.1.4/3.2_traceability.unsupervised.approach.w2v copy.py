...
def dronology_params():
    ...
    return {
        ...
        'linkType': 'req2src',
        'path_to_trained_model': '../dvc-data/systems/dronology/2vec_models/bpe128k/word2vec/word2vec_bpe128k_java_py.model',
        'source_type': 'req',
        'target_type': 'code',
        'system_path_config': {
        ...
        'saving_path': os.path.join(base, 'experiment4.1.2/results') + os.sep,
...
class Word2VecSeqVect(BasicSequenceVectorization):
    ...
    def compute_and_save(self, sampling=False, samples=None):
        ...
        for i, link in enumerate(links, 1):
            ...
            sent_a = self.df_source.loc[self.df_source['ids']==link[0],'bpe128k'].iat[0].split()
            sent_b = self.df_target.loc[self.df_target['ids']==link[1],'bpe128k'].iat[0].split()
