...
SYSTEM_CSV = '../dvc-data/systems/dronology/dronology_conv.csv'
SEP = '~'
MODEL_PATH = '../models/doc2vec/[doc2vec-py-java-PVDBOW-500-20E-32k-c-1750433324.769117].model'
MAPPING_CSV = '../dvc-data/systems/dronology/mappings/conventional.csv'
OUTPUT_DIR = '../dvc-data/systems/dronology/results/'
INFER_STEPS = 200
...
def load_dataset(system_csv, sep):
    ...
    df['conv'] = df.get('conv', '').fillna('')
    df_src = df[df.get('type') == 'req'][['ids', 'conv']].reset_index(drop=True)
    df_trg = df[df.get('type') == 'code'][['ids', 'conv']].reset_index(drop=True)
...
def infer_vectors(df, model_path, epochs):
    ...
    return pd.DataFrame({
        'ids': df['ids'],
        'vector': df['conv'].apply(lambda doc: model.infer_vector(doc.split(), epochs=epochs))
    })
