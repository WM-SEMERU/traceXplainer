...
def dronology_params():
    ...
    return {
        ...
        'linkType': 'req2src',
        'path_to_trained_model': '../dvc-data/systems/dronology/2vec_models/conv_wiki/word2vec/word2vec_java_py_wiki_conv.model',
        'source_type': 'req',
        'target_type': 'code',
        'system_path_config': {
            'system_path': os.path.join(base, 'corpus/dronology_conv_with_bpe_fixed.csv'),
            'sep': '~',
            'prep': Preprocessing.bpe,
            'names': ['ids', 'conv'],
        },
        'saving_path': os.path.join(base, 'experiment4.2.0/results') + os.sep,
...
class Word2VecSeqVect(BasicSequenceVectorization):
    def __init__(self, params, distance_metrics):
        ...
        kv = KeyedVectors.load_word2vec_format(
            params['path_to_trained_model'], binary=False
        )
        kv.fill_norms()  # normalize vectors (replaces init_sims)
        # wrap so self.model.wv[...] still works
    ...
    def compute_and_save(self, sampling=False, samples=None):
        ...
        for i, link in enumerate(links, 1):
            ...
            sent_a = self.df_source.loc[self.df_source['ids']==link[0],'conv'].iat[0].split()
            sent_b = self.df_target.loc[self.df_target['ids']==link[1],'conv'].iat[0].split()
