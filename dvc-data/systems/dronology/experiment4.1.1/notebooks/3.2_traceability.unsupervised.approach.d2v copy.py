...
SEP = '~'
MODEL_PATH = '../dvc-data/systems/dronology/2vec_models/bpe8k/doc2vec/[doc2vec-py-java-PVDBOW-500-20E-32k-c-1751384704.291919].model'
MAPPING_CSV = '../dvc-data/systems/dronology/mappings/conventional.csv'
OUTPUT_DIR = '../dvc-data/systems/dronology/experiment4.1.1/results/'
INFER_STEPS = 200
...
def load_dataset(system_csv, sep):
    ...
    df['bpe8k'] = df.get('bpe8k', '').fillna('')
    df_src = df[df.get('type') == 'req'][['ids', 'bpe8k']].reset_index(drop=True)
    df_trg = df[df.get('type') == 'code'][['ids', 'bpe8k']].reset_index(drop=True)
...
def infer_vectors(df, model_path, epochs):
    ...
    return pd.DataFrame({
        'ids': df['ids'],
        'vector': df['bpe8k'].apply(lambda doc: model.infer_vector(doc.split(), epochs=epochs))
    })
