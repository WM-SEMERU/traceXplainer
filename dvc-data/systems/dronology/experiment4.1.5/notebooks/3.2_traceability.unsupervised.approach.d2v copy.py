...
SEP = '~'
MODEL_PATH = '../dvc-data/systems/dronology/2vec_models/bpe128k/doc2vec/[doc2vec-py-java-PVDBOW-500-20E-128k-c-1751815302.13117].model'
MAPPING_CSV = '../dvc-data/systems/dronology/mappings/conventional.csv'
OUTPUT_DIR = '../dvc-data/systems/dronology/experiment4.1.5/results/'
INFER_STEPS = 200
...
def load_dataset(system_csv, sep):
    ...
    df['bpe128k'] = df.get('bpe128k', '').fillna('')
    df_src = df[df.get('type') == 'req'][['ids', 'bpe128k']].reset_index(drop=True)
    df_trg = df[df.get('type') == 'code'][['ids', 'bpe128k']].reset_index(drop=True)
...
def infer_vectors(df, model_path, epochs):
    ...
    return pd.DataFrame({
        'ids': df['ids'],
        'vector': df['bpe128k'].apply(lambda doc: model.infer_vector(doc.split(), epochs=epochs))
    })
