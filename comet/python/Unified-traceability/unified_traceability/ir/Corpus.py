...
class Corpus:
    ...
    def get_preset_corpus(cls, corpus_code):
        ...
        corpus_root = file_path + '/../../../../datasets/' + dataset_name + '_semeru_format/'

        corpus_name = dataset_name + ' (' + corpus_code + ')'
        filetype_whitelist = ['java', 'txt', 'jsp', 'h', 'c']

        corpus = Corpus(corpus_name, corpus_root, source_path, target_path, truth_path, execution_traces=execution_traces, corpus_code=corpus_code, languages=languages, filetype_whitelist=filetype_whitelist)
